...
def _get_team_ratings(data):
    ...
    ratings = (
        team_data.groupby(["entity_name", "game_id", "date"])
        .agg(
    ...
    ratings["skill_rating"] = compute_rating_lower_bound(
        ratings["skill_rating_mu"], ratings["skill_rating_sigma"]
    )
...
def _display_player_evolution(skill_ratings):
    skill_ratings["skill_rating_997%"] = compute_rating_lower_bound(
        skill_ratings["skill_rating_mu"], skill_ratings["skill_rating_sigma"]
    )
    skill_ratings["skill_rating_003%"] = compute_rating_upper_bound(
        skill_ratings["skill_rating_mu"], skill_ratings["skill_rating_sigma"]
    )
    secondary_y_axis = "pscore"

    skill_ratings["pscore_mean"] = skill_ratings.groupby("series_name")[
        "pscore"
    ].transform("mean")
    ...
    with show_settings_columns[1]:
        show_pscore = (
            st.checkbox("Show Performance Score", value=False)
            if not comparing_entities
            else False
        )
    ...
    base = alt.Chart(skill_ratings).encode(alt.X("index:Q").title("Game number"))

    nearest = alt.selection_point(
        nearest=True, on="mouseover", fields=["index"], empty=False
    )
    ...
    if show_gaussian_ratings:
        ...
        metrics_chart_mean = (
            ...
            .encode(
                x="index:Q",
                y=alt.Y("skill_rating_mu:Q").title(""),
                color="entity_name:N",
            )
            .properties(width=800, height=800)
        )
    ...
    if show_pscore:
        metrics_chart = (
            base.mark_point()
            .encode(x="index:Q", y=f"{secondary_y_axis}:Q", color="series_name:N")
            .properties(width=800, height=800)
            .interactive()
        )
    ...
    if show_pscore:
        chart = (
            alt.layer(
                rating_chart,
                alt.layer(metrics_chart_mean_area, metrics_chart, metrics_chart_mean),
                selectors,
                rules,
            )
        ...
        chart = (
            alt.layer(rating_chart, selectors, rules)
            .resolve_scale(y="independent")
            .configure_legend(titleColor="black", titleFontSize=14)
        )
