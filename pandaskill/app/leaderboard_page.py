...
def display_leaderboard_page(data):
    ...
    st.info(
        f"Leaderboard at date {date}, with at least {min_nb_games} games since {since}"
    )
    ...
    ranking_formatted = ranking.rename(
        columns={
            "rank": "Rank",
            "player_name": "Player",
            "team_name": "Team",
    ...
    st.info(
        "Player skill ratings are modeled as Gaussian distribution with parameters mu and sigma. They are ranked using the lower bound of the 99.7% confidence interval of the skill rating. See [here](https://en.wikipedia.org/wiki/68%E2%80%9395%E2%80%9399.7_rule) for more information."
    )
...
def _create_team_ranking_from_player_ranking(ranking):
    ...
    )  # so that we select the top 5 players per team
    ranking = (
        ...
        .agg(
            ...
            skill_rating_sigma=(
                "skill_rating_sigma",
                lambda x: np.sqrt(np.mean(np.square(x[:5]))),
            ),
        )
        .reset_index()
    )
    ranking["skill_rating"] = compute_rating_lower_bound(
        ranking["skill_rating_mu"], ranking["skill_rating_sigma"]
    )
...
def _display_distributions(ranking):
    ...
    df = ranking.copy()
    if x_column == "region":
        region_order_dict = {region: i for i, region in enumerate(ALL_REGIONS)}
        ...
        all_regions_formatted = [format_region(region) for region in ALL_REGIONS]
