...
def visualize_ratings(
    data_with_ratings: pd.DataFrame, experiment_dir: str, visualization_parameters: dict
) -> None:
    saving_dir = join(experiment_dir, "ratings_visualization")
    os.makedirs(saving_dir, exist_ok=True)

    _visualize_ratings_distributions(
        data_with_ratings,
        saving_dir,
        visualization_parameters["min_nb_games"],
        visualization_parameters["since"],
    )

    if "meta_rating_after" in data_with_ratings.columns:
        _visualize_meta_rating_evolution(data_with_ratings, saving_dir)
...
def _visualize_ratings_distributions(
    ...
    last_skill_ratings_df = last_skill_ratings_df[
        last_skill_ratings_df["date"] >= since
    ]

    last_skill_ratings_df = last_skill_ratings_df[
        last_skill_ratings_df["nb_games"] >= min_nb_games
    ]
    ...
    plot_violin_distributions(
        ...
        "role",
        "skill_rating_after",
        "skill rating distribution by role",
        "Role",
        "Player Skill Rating",
        saving_dir,
    ...
    plot_violin_distributions(
        ...
        "region",
        "skill_rating_after",
        "skill rating distribution by region",
        "Region",
        "Player Skill Rating",
        saving_dir,
    ...
    if "contextual_rating_after" in last_skill_ratings_df.columns:
        plot_violin_distributions(
            ...
            "Player contextual rating distribution by role",
            "Role",
        ...
        plot_violin_distributions(
            ...
            "region",
            "contextual_rating_after",
            "Player contextual rating distribution by region",
            "Region",
            "Player Contextual Skill Rating",
            saving_dir,
...
def _visualize_meta_rating_evolution(
    ...
    region_skill_ratings_after_series_df, nb_interregion_games_per_series = (
        construct_skill_ratings_for_region_after_series(ratings_df)
    )
    ...
    top_10_players = (
        sorted_df.groupby(["series_name", "region"], observed=False)
        .head(10)
        .reset_index()
    )
    _create_and_save_meta_rating_evolution(
...
def construct_skill_ratings_for_region_after_series(
    ...
    for series_name in series_names:
        series_interregion_data = interregion_ratings[
            interregion_ratings.series_name == series_name
        ]
        series_end_date = ratings_df[ratings_df.series_name == series_name][
            "date"
        ].max()
        for region in series_interregion_data.region.unique():
            ratings_in_region_after_series = (
                _get_skill_ratings_of_region_at_a_given_point_in_time(
                    ratings_df, region, series_end_date
                )
            )
            ratings_after_series = ratings_in_region_after_series.loc[
                :, ["skill_rating_after"]
            ]
        ...
        nb_interregion_games_for_series = len(
            series_interregion_data.index.get_level_values(0).unique()
        )
        nb_interregion_games.append(nb_interregion_games_for_series)

    region_skill_ratings_after_series_df = pd.concat(data)

    nb_interregion_games_df = pd.Series(nb_interregion_games, index=series_names)
...
def _get_skill_ratings_of_region_at_a_given_point_in_time(
    ratings_df: pd.DataFrame, region: str, date: str
) -> pd.DataFrame:
    """
    ...
    def recompute_ratings_for_players_in_region(
        last_rating_for_players_in_last_regular_season,
        last_region_rating_mu,
        last_region_rating_sigma,
    ):
        last_rating_for_players_in_last_regular_season["meta_rating_after_mu"] = (
            last_region_rating_mu
        )
        last_rating_for_players_in_last_regular_season["meta_rating_after_sigma"] = (
            last_region_rating_sigma
        )
        last_rating_for_players_in_last_regular_season["meta_rating_after"] = (
            lower_bound_rating(last_region_rating_mu, last_region_rating_sigma)
        )

        new_overall_ratings = last_rating_for_players_in_last_regular_season.apply(
            lambda row: combine_contextual_and_meta_ratings(
                row["contextual_rating_after_mu"],
                row["contextual_rating_after_sigma"],
                row["meta_rating_after_mu"],
                row["meta_rating_after_sigma"],
            ),
            axis=1,
        ).values
        new_overall_ratings = np.array(
            [[mu, sigma] for (mu, sigma) in new_overall_ratings]
        )
        last_rating_for_players_in_last_regular_season["skill_rating_after_mu"] = (
            new_overall_ratings[:, 0]
        )
        last_rating_for_players_in_last_regular_season["skill_rating_after_sigma"] = (
            new_overall_ratings[:, 1]
        )
        last_rating_for_players_in_last_regular_season["skill_rating_after"] = (
            last_rating_for_players_in_last_regular_season.apply(
    ...
    six_months_before_date = datetime.strptime(
        date, "%Y-%m-%d %H:%M:%S.%f"
    ) - timedelta(days=6 * 30)
    last_ratings_in_region = (
        ratings_df[
    ...
    last_region_rating_mu, last_region_rating_sigma = (
        last_ratings_in_region.tail(1)
        .loc[:, ["meta_rating_after_mu", "meta_rating_after_sigma"]]
        .values[0]
    )
    last_ratings_in_region = recompute_ratings_for_players_in_region(
        last_ratings_in_region, last_region_rating_mu, last_region_rating_sigma
    )
...
def _create_and_save_meta_rating_evolution(
    ...
    for region in ALL_REGIONS:
        region_data = ratings_in_region_after_series[
            ratings_in_region_after_series["region"] == region
        ]
        color = color_dict[region]
        marker = marker_dict[region]

        mean_data = (
            region_data.groupby("series_name", observed=False)["skill_rating_after"]
            .mean()
            .reset_index()
        )
    ...
    plt.savefig(
        join(saving_dir, f"{file_name[:-4]}.pdf"), format="pdf", bbox_inches="tight"
    )
