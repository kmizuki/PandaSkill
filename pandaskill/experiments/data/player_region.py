...
MAIN_LEAGUE_SERIES_TOURNAMENT_WHITELIST = {
    ...
    "Latin America": {
        f"LLA {season} {year}": (
            ["Phase 1"] if year in [2020, 2021] else ["Regular Season"]
        )
        for year in range(2019, 2025)
...
regular_season_team_names_before_data = {
    "Korea": [
        ...
        "Hanwha Life Esports",
        "Liiv SANDBOX",
    ],  # exclude Jin Air Green Wings as they've demoted to Challengers Korea
    "China": [
        "Royal Never Give Up",
        "JD Gaming",
        "Oh My God",
...
def attribute_player_in_game_to_region(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mask_df = pd.DataFrame(
        regular_season_tournaments, columns=["series_name", "tournament_name"]
    )
    regular_season_df = (
        df.reset_index()
        .merge(mask_df, on=["series_name", "tournament_name"])
        .set_index(["game_id", "player_id"])
    )

    main_regions_series_participants_df = regular_season_df.groupby("series_name").agg(
        {
            "league_name": lambda x: x.iloc[0],
            "date": lambda x: x.iloc[0],
            "team_id": lambda team_id_for_series_series: list(
                team_id_for_series_series.unique()
            ),
        }
    )
    main_regions_series_participants_df = (
        main_regions_series_participants_df.sort_values("date")
    )
...
def _create_main_regions_participants_lookup(
    df: pd.DataFrame,
    region_rosters_df: pd.DataFrame,
    absolute_start_date: str,
    absolute_end_date: str,
) -> dict:
    lookup = {}
    ...
    for region, team_names in regular_season_team_names_before_data.items():
        for team_name in team_names:
            if team_name not in team_name_to_id_map:
                logging.warning(
                    f"Team {team_name} from region {region} not found in the dataset"
                )
            else:
...
def manually_correct_team_region(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[
        ...
        "region",
    ] = "Other"  # they don't participate in the LEC Spring split that starts few days later

    df.loc[
        (df.series_name == "LMF Opening 2023") & (df.team_name == "Globant Emerald"),
        "region",
    ] = "Other"  # they did not qualify to the LLA Opening 2023

    return df


def _fix_kespa_cup_regions(df: pd.DataFrame) -> pd.DataFrame:
    df.loc[(df.series_name == "KeSPA Cup 2019") & (df.team_name == "T1"), "region"] = (
        "Korea"  # SK telecom T1 renamed to T1
    )
    df.loc[(df.series_name == "KeSPA Cup 2019") & (df.team_name == "DRX"), "region"] = (
        "Korea"  # Kingzone DragonX renamed to DRX
    )

    df.loc[
        (df.series_name == "KeSPA Cup 2020") & (df.team_name == "Nongshim Red Force"),
        "region",
    ] = "Korea"  # Team Dynamics renamed to Nongshim Red Force
    df.loc[
        (df.series_name == "KeSPA Cup 2020") & (df.team_name == "BRION"), "region"
    ] = "Korea"  # Brion Blade renamed to BRION
...
def _fix_demacia_cup_regions(df: pd.DataFrame) -> pd.DataFrame:
    df.loc[
        (df.series_name == "Demacia Cup 2019") & (df.team_name == "FunPlux Phoenix"),
        "region",
    ] = "Other"  # FunPlus Phoenix plays with academy roster
    df.loc[
        (df.series_name == "Demacia Cup 2019") & (df.team_name == "Invictus Gaming"),
        "region",
    ] = "Other"  # Invictus Gaming plays with academy roster + new players

    df.loc[
        (df.series_name == "Demacia Cup 2020") & (df.team_name == "ThunderTalk Gaming"),
        "region",
    ] = "China"  # Dominus Esports renamed to ThunderTalk Gaming

    df.loc[
        (df.series_name == "Demacia Cup 2021") & (df.team_name == "Anyone's Legend"),
        "region",
    ] = "China"  # Rogue Warrors renamed to Anyone's Legend
    df.loc[
        (df.series_name == "Demacia Cup 2021") & (df.team_name == "Weibo Gaming"),
        "region",
    ] = "China"  # Suning renamed to Weibo Gaming
...
def _attribute_region_to_all_star_2020(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for team_name, region in team_name_region_map.items():
        df.loc[
            (df.series_name == "All-Star 2020") & (df.team_name == team_name), "region"
        ] = region
...
def _attribute_region_to_showmatches_2023(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for series_name, region in series_name_to_region_map.items():
        df.loc[(df.series_name == series_name), "region"] = region
