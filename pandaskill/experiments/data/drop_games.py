...
def _drop_specific_games(stat_df: pd.DataFrame) -> Tuple[pd.DataFrame, list]:
    ...
    stat_df = stat_df[~stat_df.index.get_level_values("game_id").isin(game_ids_to_drop)]
...
def _drop_incomplete_games(stat_df: pd.DataFrame) -> Tuple[pd.DataFrame, dict]:
    ...
    stat_df, wrong_game_length_dropped_games = _drop_rows_with_wrong_value(
        stat_df, "game_length", 0
    )
    stat_df, wrong_gold_earned_dropped_games = _drop_rows_with_wrong_value(
        stat_df, "gold_earned", 0
    )
    stat_df, wrong_total_damage_dealt_to_champions_dropped_games = (
        _drop_rows_with_wrong_value(stat_df, "total_damage_dealt_to_champions", 0)
    )
    stat_df, wrong_total_damage_taken_dropped_games = _drop_rows_with_wrong_value(
        stat_df, "total_damage_taken", 0
    )
...
def _drop_rows_with_wrong_value(
    ...
    feature_wrong_value_dropped_game_ids = (
        stat_df[feature_wrong_value_mask]
    ...
    logging.info(
        f"Dropping {len(feature_wrong_value_dropped_game_ids)} games due to \
...
def _drop_games_with_nans(stat_df: pd.DataFrame) -> Tuple[pd.DataFrame, list]:
    ...
    logging.info(
        f"Dropping {len(nan_cells_dropped_game_ids)} games due to NaN cells: \
        {nan_cells_dropped_game_ids}"
    )
...
def _drop_games_with_missing_rows(stat_df: pd.DataFrame) -> Tuple[pd.DataFrame, list]:
    mask_with_10_rows_per_game = (
        stat_df.reset_index("game_id").groupby("game_id").count() == 10
    )
    mask_with_10_rows_per_game = mask_with_10_rows_per_game[
        mask_with_10_rows_per_game["date"]
    ]
    complete_game_ids = mask_with_10_rows_per_game.index
    rows_to_keep_mask = stat_df.index.get_level_values("game_id").isin(
        complete_game_ids
    )
    missing_rows_dropped_game_ids = (
        stat_df[~rows_to_keep_mask]
        .index.get_level_values("game_id")
        .unique()
        .values.astype(int)
        .tolist()
    ...
    logging.info(
        f"Dropping {len(missing_rows_dropped_game_ids)} games due to missing rows: \
        {missing_rows_dropped_game_ids}"
    )
