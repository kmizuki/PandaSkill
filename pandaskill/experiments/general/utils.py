...
def function_representer(
    dumper: yaml.Dumper, data: types.FunctionType
) -> yaml.ScalarNode:
    if isinstance(data, types.FunctionType):
        return dumper.represent_scalar(
            "!python/name:" + data.__module__ + "." + data.__name__, ""
        )
...
def load_data(
    ...
    game_metadata_df = pd.read_csv(
        join(raw_data_folder, "game_metadata.csv"), index_col=0
    )
    game_players_stats_df = pd.read_csv(
        join(raw_data_folder, "game_players_stats.csv"), index_col=(0, 1)
    )
    data = game_players_stats_df.join(game_metadata_df, on="game_id", how="left")

    if load_features:
        game_features_df = pd.read_csv(
            join(ARTIFACTS_DIR, "data", "preprocessing", "game_features.csv"),
            index_col=(0, 1),
        )
