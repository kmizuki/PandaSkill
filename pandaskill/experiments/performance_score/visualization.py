...
def _plot_feature_importance_per_role(
    features_weights: dict, saving_folder: str
) -> None:
...
def plot_all_models_calibration(
    calibration_data: list[dict[str, list[int]]], saving_dir: str
) -> None:
    ...
    for role in roles:
        fraction_of_positives, mean_predicted_value = calibration_curve(
            y_trues[role], y_probs[role], n_bins=25
        )

        ax1.plot(
            mean_predicted_value, fraction_of_positives, "s-", label="%s" % (role,)
        )

        ax2.hist(
            y_probs[role], range=(0, 1), bins=25, label=role, histtype="step", lw=2
        )
...
def plot_shap_game_features_impact(
    ...
    if show_xlabel:
        fig.axes[0].set_xlabel(
            "SHAP Values", labelpad=20, fontsize=14
        )  # Set xlabel for the last axis (waterfall plot)
    fig.tight_layout()
    plt.savefig(join(saving_folder, file_name[:-4] + ".pdf"), bbox_inches="tight")
    plt.close()
...
def plot_multiple_shap_features_impact(
    ...
    for idx, role in enumerate(roles):
        ...
        shap_explanation = shap.Explanation(values=shap_values, data=feature_values)
        beeswarm(
            shap_explanation,
            show=False,
    ...
    plt.savefig(
        os.path.join(saving_folder, file_name[:-4] + ".pdf"), bbox_inches="tight"
    )
    plt.close()
