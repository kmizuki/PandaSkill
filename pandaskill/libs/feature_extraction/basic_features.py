...
def compute_other_team_stat_from_team_stat(
    df: pd.DataFrame, team_stat_name: str
) -> pd.Series:
    """Get for players of one team, the value of a team stat for the other team."""
    game_totals_game_id_mapping = (
        df.groupby("game_id")[team_stat_name].sum() // 5
    ).to_dict()
    df["game_total"] = (
        df.reset_index("game_id")["game_id"].map(game_totals_game_id_mapping).values
    )
...
def compute_stat_per_total_kills_per_gold(
...
def _compute_team_stat_from_players_stat(
    df: pd.DataFrame, player_stat_name: str
) -> pd.Series:
