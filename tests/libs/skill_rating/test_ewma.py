...
from pandaskill.libs.skill_rating.ewma import (
    _compute_ewma_ratings_for_player,
    compute_ewma_ratings,
)
...
def test_compute_ewma_ratings():
    ...
    pd.testing.assert_frame_equal(
        ratings_df.reset_index(drop=True), expected_df.reset_index(drop=True)
    )
...
def test__compute_ewma_ratings_for_player():
    ...
    player_game_performance_score["date"] = pd.to_datetime(
        player_game_performance_score["date"]
    )
    player_game_performance_score = player_game_performance_score.set_index("date")

    alpha = 0.5
    skill_ratings = _compute_ewma_ratings_for_player(
        player_game_performance_score, alpha
    )
